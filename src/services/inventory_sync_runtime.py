...
def _snapshot_listening_message(
    ...
    *,
    ...
    if summary is None:
        return tr("等待进入游戏并接收完整背包")
    source = summary.get("source")
    if is_visual_inventory_source(source):
        return tr("当前为视觉扫描库存，正在等待原生背包同步；视觉扫描不提供角色实例")
    if not has_native_inventory_uids(source):
        return tr("当前库存来源尚不支持原生角色身份，正在等待原生背包同步")
    prefix = tr("已收到原生背包") if received else tr("当前已保存原生背包")
    if not has_character_list:
        return tr("{prefix}，该快照未附带独立角色列表；正在后台监听更新", prefix=prefix)
    if not summary.get("character_instance_count"):
        return tr("{prefix}，尚未观测到角色实例；正在后台监听更新", prefix=prefix)
    return tr("背包已同步，正在后台监听变化")
        ...
        with service._open_dao() as dao:
            ...
            while not service._stop_requested.is_set() and not service._capture_ready.wait(
                ...
                if time.monotonic() >= capture_ready_deadline:
                    raise TimeoutError(tr("nte-core 抓包初始化超时，未进入 running 状态"))
