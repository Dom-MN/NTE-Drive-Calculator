...
class BattleMarginalCharacterPanel(QFrame):
    ...
    def render(
        ...
        *,
        ...
        headers = tuple(
            tr("{label}（本系）", label=tr(row.label))
            if row.property_id == current_element_property else tr(row.label)
            for row in properties
        )
        self.table.setHorizontalHeaderLabels((tr("属性"), *headers))
        self.table.setColumnWidth(0, 100)
        header_metrics = self.table.horizontalHeader().fontMetrics()
        for row_index, label in enumerate((tr("静态面板"), tr("动态面板"))):
        ...
        for column, row in enumerate(properties, start=1):
            ...
            for row_index, value in enumerate((
                ...
                if row_index == 1 and value is None:
                    item.setToolTip(
                        tr("该属性没有可安全归属的公式面板伤害或动态投影。")
                    )
                self.table.setItem(row_index, column, item)
            self.table.setColumnWidth(
                column,
                max(112, header_metrics.horizontalAdvance(headers[column - 1]) + 24),
            )
